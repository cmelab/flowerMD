...
def get_molecule_backbone_vector(coordinates):
    # Ensure the input array has the correct shape
    if coordinates.shape[1] != 3:
        raise ValueError("Input array must have 3 columns (3D coordinates)")
    # Augment the coordinates with a column of ones for the intercept term
    augmented_coordinates = np.hstack(
        [coordinates, np.ones((coordinates.shape[0], 1))]
    )
    # Use the least squares method to find the parameters of the best-fit line
    params, _, _, _ = np.linalg.lstsq(
        augmented_coordinates[:, :3], coordinates[:, 2], rcond=None
    )
    # The parameters represent the direction vector of the best-fit line
    direction_vector = params[:3]
    return direction_vector


def get_molecule_backbone_vector_pca(coordinates):
    # Ensure the input array has the correct shape
    if coordinates.shape[1] != 3:
        raise ValueError("Input array must have 3 columns (3D coordinates)")
...
def get_molecule_normal_vector(coordinates):
    # Ensure the input array has the correct shape
    if coordinates.shape[1] != 3:
        raise ValueError("Input array must have 3 columns (3D coordinates)")
    # Augment the coordinates with a column of ones for the intercept term
    ...
    # Use the least squares method to find the parameters of the best-fit plane
    params, _, _, _ = np.linalg.lstsq(
        augmented_coordinates[:, :3], coordinates[:, 2], rcond=None
    )
    # The parameters represent the normal vector of the best-fit plane
