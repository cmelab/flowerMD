...
def load_isotropic_custom_force(
    ...
    """
    Load the isotropic custom force for coarse-grained molecular dynamics.

    Parameters:
    -----------
    model_config : LJNeighborModelConfig
        Configuration for the Lennard-Jones neighbor model, including details like
        number of layers, hidden dimensions, activation type, box length, and
        number of neighbors.
    best_model_path : str
        Path to the checkpoint file containing the pre-trained model weights.

    Returns:
    --------
    IsotropicCustomForce
        A force object for use in HOOMD simulations, initialized with the pre-trained model.
    """
